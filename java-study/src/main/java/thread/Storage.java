		...
		synchronized (list){
			...
			while(list.size()+num > MAX_SIZE){
				...
				try {
					//条件不满足，生产阻塞
					list.wait();
					System.out.println ("生产者 "+thread.getName ()+" 被唤醒");
			...
			System.out.println(thread.getName ()+"【已经生产产品数】:" + num + "\t【现仓储量为】:" + list.size());
	...
	public void consume(int num,Thread thread){
		synchronized (list){
			...
			while(num > list.size()){
				...
				try {
					list.wait();
					System.out.println ("消费者 "+thread.getName ()+" 被唤醒");
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
			}
			...
			System.out.println(thread.getName ()+"【已经消费产品数】:" + num + "\t【现仓储量为】:" + list.size());
